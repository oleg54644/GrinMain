# grinmain.py
import eventlet
eventlet.monkey_patch()  # обязательно до всех остальных импортов
...
# (весь HTML_TEMPLATE остаётся без изменений, поэтому здесь он опущен для краткости,
# но в реальном файле его нужно вставить полностью)
...
if __name__ == '__main__':
    ...
    # Создаём сокет и запускаем сервер в фоновом гринлете
    ...
    try:
        ...
        # Принудительно приводим к HTTPS, если cloudpub вернул tcp://
        if public_url.startswith('tcp://'):
            # Извлекаем хост (убираем порт, если он есть)
    ...
    try:
        # Ожидаем завершения сервера (например, по Ctrl+C)
