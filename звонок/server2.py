...
<html>
<head><meta charset="UTF-8"><title>📞 Голосовой чат GrinMain</title>
<style>
body {
    font-family: Arial;
    margin: 0;
...
<div id="main">
    ...
    <div>
        <input type="text" id="targetId" placeholder="ID друга"><br>
...
const chatHistoryKey = 'voiceChatHistory';
let chatHistory = [];
...
function addToChatHistory(id) {
    if (!id || id === myId) return;
    const existing = chatHistory.find(c => c.id === id);
...
function renderChatList() {
    ...
    if (chatHistory.length === 0) {
        container.innerHTML = '<p style="color:#777;">Нет завершённых звонков</p>';
        return;
    }
    let html = '';
    chatHistory.forEach(chat => {
        const time = new Date(chat.lastCalled).toLocaleString();
        html += `
        <div class="chat-item" data-id="${chat.id}">
            <div>
                <div class="id">${chat.id}</div>
                <div class="time">Последний звонок: ${time}</div>
            </div>
            <button class="call-btn" onclick="callContact('${chat.id}')">📞</button>
        </div>`;
    });
    ...
    ws.onmessage = async (e) => {
        try {
            ...
            if (d.type === 'your_id') {
                ...
                updateStatus('Готов! ID: ' + myId);
            } else if (d.type === 'offer') {
                await handleOffer(d);
            } else if (d.type === 'answer') {
                await handleAnswer(d);
            } else if (d.type === 'ice_candidate') {
                await handleIceCandidate(d);
            }
        } catch (e) {
            console.error(e);
        }
...
async function handleOffer(data) {
    ...
    addToChatHistory(data.sender_id);
    if (!localStream) {
        try {
            localStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
        } catch {
            return alert('Нет микрофона');
        }
    }
...
function updateStatus(msg) {
    document.getElementById('callStatus').textContent = 'Статус: ' + msg;
}
...
def generate_random_id():
    """Генерирует 8-значный цифровой ID"""
