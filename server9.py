...
# ========== HTML страница с исправленным двусторонним звуком ==========
HTML_PAGE = """<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>📞 Голосовой чат (двусторонний звук)</title>
    <style>
        body {
...
<body>
    <h1>📞 Голосовой чат (двусторонний звук)</h1>
    ...
    <script>
        ...
        // ========== Создание PeerConnection (только addTrack, без addTransceiver) ==========
        ...
        async function startCall() {
            ...
            try {
                ...
                // Создаём предложение (без устаревших опций)
        ...
        async function handleIceCandidate(data) {
            // Добавляем кандидат только если есть remoteDescription
            if (peerConnection && peerConnection.remoteDescription) {
                ...
                console.log('⏳ ICE кандидат получен до remoteDescription, будет обработан позже (браузер сам закеширует)');
            }
        }
...
async def main():
    print("=" * 60)
    print("🎧 ГОЛОСОВОЙ ЧАТ С CLOUDPUB (ДВУСТОРОННИЙ ЗВУК)")
    print("=" * 60)
